...
export function OptionPill({ label, emoji, selected, onSelect, ariaLabel }: OptionPillProps) {
  return (
    <button
      ...
      onClick={onSelect}
      className={`inline-flex items-center gap-2 rounded-full border px-5 py-2.5 text-[0.9375rem] font-medium shadow-md transition focus:outline-none focus-visible:ring-2 focus-visible:ring-[#138A3C] focus-visible:ring-offset-2 ${
        selected
    ...
    return (
      ...
      >
        ...
        <span className="min-w-0 flex-1">
          ...
          {helper ? <span className="mt-0.5 block text-[0.75rem] leading-snug text-[#4B5563]">{helper}</span> : null}
        </span>
        {selected ? (
