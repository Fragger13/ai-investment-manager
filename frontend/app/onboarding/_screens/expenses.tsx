              ...
              return (
                <div key={index} className="flex items-center justify-between gap-3 rounded-xl border border-[#E5E7EB] bg-[#FAFAFA] p-3 shadow-md">
                  <div className="min-w-0">
                    ...
                    <p className="mt-0.5 text-[0.8125rem] text-[#4B5563]">{loan?.productType || "—"} · {monthly ? `${formatINR(monthly)} / month` : "Add start + end dates"}</p>
                  </div>
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[88vh] overflow-y-auto">
        ...
        <div className="mt-4 rounded-lg border border-[#E5E7EB] bg-[#FAFAFA] p-3">
          <Label className="text-[0.8125rem] uppercase tracking-wide text-[#4B5563]">Total loan / EMI amount</Label>
          ...
          {monthlyEmi > 0 && months > 0 ? (
            <p className="mt-0.5 text-[0.75rem] text-[#4B5563]">{months} months × {formatINR(monthlyEmi)}</p>
          ) : null}
          {monthlyEmi > 0 && draft.endDate ? (
            <p className="mt-1 text-[0.75rem] text-[#4B5563]">Changing this amount adjusts the start date based on your EMI and end date.</p>
          ) : null}
        </div>
